...
from agentes.documentos import gerar_requisicao, gerar_dfd, gerar_etp, gerar_tr, gerar_lista_verificacao_adesao_ata, gerar_processo_completo
...
def main():
    st.set_page_config(page_title="Agente Compras IA - Build 1.0 RC32", layout="wide")
    init_db()

    st.title("Agente Compras IA — Build 1.0 RC32")
    ...
    with aba1:
        ...
        tipo = st.selectbox("Tipo de contratação", ["Contratação artística", "Adesão à Ata de Registro de Preços", "Estrutura de evento", "Geral"])
        ...
        if st.button("Analisar Documentação Externa", type="primary"):
            base = "Contratação de show artístico" if tipo == "Contratação artística" else "Processo administrativo de adesão à Ata de Registro de Preços" if tipo == "Adesão à Ata de Registro de Preços" else "Contratação de palco e som" if tipo == "Estrutura de evento" else "Contratação geral"
    ...
    st.divider()
    st.caption("Build 1.0 RC32 — Base Homologada + Revisor IA via API. A IA revisa e sugere; a homologação continua manual.")
