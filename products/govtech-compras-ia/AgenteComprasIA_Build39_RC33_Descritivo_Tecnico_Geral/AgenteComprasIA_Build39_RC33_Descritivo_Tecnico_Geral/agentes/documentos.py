...
def objeto_padrao(dados):
    if eh_esculturas_tematicas(dados, {}):
        return (
            "Contratação de empresa especializada para fabricação, fornecimento, transporte e instalação de esculturas temáticas "
            "decorativas, educativas, turísticas e cenográficas em fibra de vidro reforçada, destinadas à implantação de atrativos "
            "turísticos, culturais, ambientais e de lazer em espaços públicos municipais."
        )
...
# =========================
# DESCRITIVO TÉCNICO DO OBJETO
# =========================

def eh_esculturas_tematicas(dados=None, classificacao=None):
    texto = " ".join([
        str((dados or {}).get("descricao", "")),
        str((classificacao or {}).get("tipo", "")),
        str((classificacao or {}).get("categoria", "")),
        str((classificacao or {}).get("objeto_resumido", "")),
    ]).lower()
    termos = [
        "escultura", "esculturas", "fibra de vidro", "grama sintética", "grama sintetica",
        "dinossauro", "pré-histórico", "pre-historico", "cenográfico", "cenografico",
        "atrativo turístico", "atrativo turistico", "peças temáticas", "pecas tematicas",
        "mobiliário temático", "mobiliario tematico", "parque temático", "parque tematico",
    ]
    return any(t in texto for t in termos)



def descritivo_tecnico_esculturas_texto(dados=None):
    return [
        (
            "Objeto: Contratação de empresa especializada para fabricação, fornecimento, transporte e instalação "
            "de esculturas temáticas decorativas, educativas, turísticas e cenográficas em fibra de vidro reforçada, "
            "destinadas à implantação de atrativos turísticos, culturais, ambientais e de lazer em espaços públicos municipais."
        ),
        (
            "As esculturas deverão ser produzidas com estrutura interna metálica reforçada, modelagem artística tridimensional, "
            "revestimento em fibra de vidro de alta resistência, acabamento técnico e artístico, tratamento para uso externo "
            "e proteção contra intempéries, garantindo durabilidade, segurança e qualidade estética."
        ),
        (
            "O projeto poderá contemplar esculturas temáticas de animais da fauna brasileira e mundial, animais pré-históricos, "
            "elementos ambientais, peças decorativas, esculturas revestidas com grama sintética, estruturas interativas para fotografia, "
            "elementos cenográficos, mobiliários temáticos e demais composições compatíveis com a proposta turística e cultural do município."
        ),
        "As peças deverão apresentar:",
        [
            "Estrutura interna metálica resistente;",
            "Modelagem artística tridimensional;",
            "Revestimento em fibra de vidro reforçada;",
            "Acabamento artístico de alta qualidade;",
            "Pintura especial ou revestimento em grama sintética, conforme projeto;",
            "Proteção UV para exposição permanente em ambientes externos;",
            "Resistência à chuva, sol e variações climáticas;",
            "Fixação segura e adequada ao local de instalação;",
            "Ausência de arestas cortantes ou elementos que ofereçam risco aos usuários.",
        ],
        (
            "As dimensões poderão variar conforme o projeto executivo, compreendendo esculturas de pequeno, médio e grande porte, "
            "destinadas à composição de parques temáticos, praças, jardins, centros culturais, áreas de lazer, espaços educativos "
            "e atrativos turísticos municipais."
        ),
        (
            "A contratação visa promover a valorização dos espaços públicos, fomentar o turismo local, incentivar atividades educativas "
            "e culturais, ampliar as opções de lazer da população e fortalecer a identidade turística do município por meio da implantação "
            "de estruturas temáticas permanentes de interesse coletivo."
        ),
    ]


def descritivo_tecnico_generico_texto(dados=None, classificacao=None):
    dados = dados or {}
    classificacao = classificacao or {}
    objeto = clean(dados.get("descricao")) or clean(classificacao.get("objeto_resumido")) or "Contratação de bens, serviços ou solução técnica conforme necessidade administrativa da unidade requisitante"
    secretaria = clean(dados.get("secretaria")) or "unidade requisitante"

    return [
        f"Objeto: {objeto}.",
        (
            "O objeto deverá ser executado ou fornecido por empresa especializada, com capacidade técnica compatível com a natureza da contratação, "
            "observadas as especificações mínimas, condições de qualidade, segurança, entrega, execução, garantia e demais requisitos definidos no Termo de Referência, "
            "no Estudo Técnico Preliminar, na proposta e nos documentos de instrução do processo."
        ),
        (
            "A solução deverá atender à finalidade pública pretendida pela Administração, respeitando os princípios da eficiência, economicidade, planejamento, "
            "segurança, durabilidade, funcionalidade, adequação técnica e compatibilidade com o interesse público envolvido."
        ),
        (
            "O escopo poderá contemplar fornecimento, prestação de serviços, instalação, montagem, transporte, configuração, adequação, suporte, assistência técnica, "
            "garantia, treinamento ou demais providências necessárias ao pleno atendimento da necessidade administrativa, conforme a natureza do objeto e as condições estabelecidas no processo."
        ),
        "Os bens, serviços ou soluções deverão apresentar, quando aplicável:",
        [
            "Conformidade com as especificações técnicas descritas no Termo de Referência;",
            "Qualidade compatível com a finalidade pública pretendida;",
            "Materiais, componentes, mão de obra, equipamentos ou métodos adequados à execução do objeto;",
            "Entrega, execução ou instalação conforme prazos e locais definidos pela Administração;",
            "Segurança aos usuários, servidores, operadores e ao público envolvido;",
            "Ausência de vícios, defeitos, riscos indevidos ou características incompatíveis com o uso pretendido;",
            "Garantia, assistência técnica ou suporte, quando exigidos pela natureza do objeto;",
            "Observância às normas técnicas, sanitárias, ambientais, de acessibilidade, segurança do trabalho ou regulamentos aplicáveis, quando pertinentes;",
            "Compatibilidade com o projeto, memorial, layout, especificações complementares ou demais anexos do processo, quando existentes.",
        ],
        (
            "As quantidades, dimensões, unidades, padrões de acabamento, locais de entrega ou execução, condições de instalação, prazos, obrigações da contratada "
            "e critérios de recebimento deverão ser definidos ou complementados no Termo de Referência, no projeto executivo, no memorial descritivo ou nos anexos técnicos correspondentes."
        ),
        (
            f"A contratação visa atender à necessidade da {secretaria}, garantindo melhores condições de funcionamento dos serviços públicos, atendimento à população, "
            "execução das políticas públicas municipais, preservação do interesse coletivo e adequada aplicação dos recursos públicos."
        ),
    ]


def descritivo_tecnico_texto(dados=None, classificacao=None):
    if eh_esculturas_tematicas(dados, classificacao):
        return descritivo_tecnico_esculturas_texto(dados)
    return descritivo_tecnico_generico_texto(dados, classificacao)

def texto_descritivo_tecnico_plain(dados=None, classificacao=None):
    blocos = descritivo_tecnico_texto(dados, classificacao)
    partes = ["DESCRITIVO TÉCNICO DO OBJETO", ""]
    for b in blocos:
...
def gerar_descritivo_tecnico(processo_id, dados, classificacao, pasta_saida: Path, extraidos=None):
    dados = enriquecer(dados, extraidos)
    pasta_saida.mkdir(parents=True, exist_ok=True)
    arquivo = pasta_saida / "00_Descritivo_Tecnico_Objeto.docx"
    ...
    add_bold(doc, "DESCRITIVO TÉCNICO DO OBJETO")
    add_p(doc, "")

    for bloco in descritivo_tecnico_texto(dados, classificacao):
        if isinstance(bloco, list):
            for item in bloco:
                ...
                run.font.name = FONTE_PADRAO
                run.font.size = Pt(11)
        elif bloco == "As peças deverão apresentar:":
            add_bold(doc, bloco)
        else:
            add_p(doc, bloco)
    ...
    add_p(doc, resp.upper() if resp else "RESPONSÁVEL PELA DEMANDA", style="assinatura")
    add_p(doc, cargo.upper() if cargo else "CARGO/FUNÇÃO", style="assinatura")

    aplicar_layout_documento(doc, "DESCRITIVO TÉCNICO")
    doc.save(arquivo)
    return arquivo
...
def gerar_processo_completo(processo_id, dados, classificacao, pasta_saida: Path, extraidos=None):
    ...
    arquivos = []
    if eh_esculturas_tematicas(dados, classificacao):
        arquivos.append(gerar_descritivo_tecnico(processo_id, dados, classificacao, pasta_saida, extraidos))
    arquivos.extend([
