...
from agentes.documentos import gerar_requisicao, gerar_dfd, gerar_etp, gerar_tr, gerar_lista_verificacao_adesao_ata, gerar_descritivo_tecnico, gerar_processo_completo
...
from agentes.base_conhecimento import resumo_referencias, buscar_referencias
...
def main():
    st.set_page_config(page_title="Agente Compras IA - Build 1.0 RC33", layout="wide")
    init_db()

    st.title("Agente Compras IA — Build 1.0 RC33")
    ...
    with aba1:
        ...
        tipo = st.selectbox("Tipo de contratação", ["Contratação artística", "Compra com Descritivo Técnico", "Esculturas temáticas / Fibra de Vidro", "Adesão à Ata de Registro de Preços", "Estrutura de evento", "Geral"])
        ...
        if st.button("Analisar Documentação Externa", type="primary"):
            base = "Contratação de show artístico" if tipo == "Contratação artística" else "Compra com descritivo técnico do objeto" if tipo == "Compra com Descritivo Técnico" else "Contratação de empresa especializada para fabricação, fornecimento, transporte e instalação de esculturas temáticas em fibra de vidro reforçada" if tipo == "Esculturas temáticas / Fibra de Vidro" else "Processo administrativo de adesão à Ata de Registro de Preços" if tipo == "Adesão à Ata de Registro de Preços" else "Contratação de palco e som" if tipo == "Estrutura de evento" else "Contratação geral"
    ...
    with aba3:
        ...
        consulta_base = " ".join([
            dados.get("descricao", ""),
            classificacao.get("tipo", ""),
            classificacao.get("modalidade", ""),
            "adesão ata registro preços lei 14.133 checklist" if "Ata" in classificacao.get("tipo", "") else "",
        ])
        refs_base = buscar_referencias(consulta_base, limite=3)
        if refs_base:
            with st.expander("Referências encontradas na Base de Conhecimento"):
                for ref in refs_base:
                    st.write(f"**{ref.get('arquivo')}** — {ref.get('tipo')} — score {ref.get('score')}")
                    trecho = " ".join((ref.get("texto") or "").split())[:1000]
                    if trecho:
                        st.caption(trecho)
        ...
        with b:
            if st.button("Gerar Descritivo"):
                baixar_doc("Descritivo Técnico", gerar_descritivo_tecnico, dados, classificacao, ext)
        with c:
        ...
        with f:
            if st.button("Gerar Checklist AGU Ata"):
                baixar_doc("Lista de Verificação AGU", gerar_lista_verificacao_adesao_ata, dados, classificacao, ext)
        with g:
    ...
    st.divider()
    st.caption("Build 1.0 RC33 — Base Homologada + Revisor IA via API. A IA revisa e sugere; a homologação continua manual.")
